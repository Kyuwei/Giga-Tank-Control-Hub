  ...
  return (
    <div className="flex items-center justify-center w-full h-screen bg-black select-none p-2 sm:p-4">
      ...
      >
        ...
        {/* Main Area : MFD button grid + live telemetry sidebar */}
        <div className="flex-1 flex gap-2 md:gap-4 overflow-hidden mt-4">
          {/* MFD Buttons Grid */}
          <div className="flex-1 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-2 md:gap-4 overflow-auto pb-2 pr-2 content-start">
            <MfdButton
              icon={<Power size={48} />}
              label="ENGINE"
          ...
          <div className="w-44 shrink-0 flex flex-col gap-3 overflow-hidden">
            ...
            <div className={`border-2 p-3 flex items-center justify-between ${
              stabOn ? "border-[#39ff14]/60 bg-[#39ff14]/10" : "border-[#39ff14]/20 bg-[#0a1a0a]"
            }`}>
              <span className="text-[#39ff14]/50 text-xs tracking-widest flex items-center gap-1">
                <ShieldAlert size={12} /> STAB
              </span>
              <span className={`font-bold text-sm tracking-widest ${stabOn ? "text-[#39ff14]" : "text-[#39ff14]/20"}`}>
                {wtOnline ? (stabOn ? "ARMED" : "OFF") : "—"}
              </span>
            </div>
          </div>
        </div>
